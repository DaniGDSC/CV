...
# Hàm trộn hai ảnh (blending) bằng alpha blending
...
def create_panorama(img1, img2):
    ...
    # Kiểm tra xem có đủ descriptor để ghép nối không
    if descriptors1 is None or descriptors2 is None:
        raise ValueError("Không tìm thấy điểm đặc trưng trong một hoặc cả hai ảnh!")
    ...
    # Kiểm tra số lượng cặp ghép nối
    if len(good_matches) < 4:  # Cần ít nhất 4 cặp để tính homography
        raise ValueError(f"Không đủ cặp ghép nối tốt để tính homography (chỉ có {len(good_matches)} cặp)!")
    ...
    return panorama, H, len(good_matches)
...
def main():
    ...
    try:
        # Tạo ảnh panorama
        panorama, H, num_matches = create_panorama(img1, img2)
        ...
        # In thông tin
        print(f"Số lượng cặp ghép nối tốt: {num_matches}")
